...
public partial class WNZ : System.Web.UI.Page
{
    ...
    {
        SqlConnection con = new SqlConnection(@"Data Source=(local);Initial Catalog=KT-all1;Integrated Security=True");
        SqlCommand cmd = new SqlCommand("Select * from ksiazka_telefoniczna WHERE Wydzial = 'Wydział Nauk o Zdrowiu'", con);
    ...
    private void SearchPhoneBookRecords()
    {
        ...
        if (con.State == ConnectionState.Closed)
            con.Open();
        SqlDataAdapter sqlDa = new SqlDataAdapter("ViewOrSearchWNZ", con);
